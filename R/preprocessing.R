...
tb_preprocessing <- function(tb_df, cutoff = 95, impute = FALSE) {

  # define binary outcome
  tb_df$adherence_outcome <- ifelse(tb_df$PCTadherence_sensi < cutoff, 1, 0)
  ...
  tb_df$family_median <- apply(tb_df[, fam_vars], 1, floor_median)
  ...
  motivation_vars <- c("motiv_no_transmit", "motiv_fam_worry", "motiv_study_work",
                       "motiv_activities")
  tb_df$motiv_median <- apply(tb_df[, motivation_vars], 1, floor_median)
  tb_df <- tb_df %>% select(-all_of(c(motivation_vars,  "motiv_summary")))

  # tb disinformation
  knowledge_vars <- c("conoc_cure", "conoc_missed_doses", "conoc_default")
  tb_df$knowledge_median <- apply(tb_df[, knowledge_vars], 1, floor_median)
  ...
  na_df <- tb_df[rowSums(is.na(tb_df)) > 0,colSums(is.na(tb_df)) > 0]
